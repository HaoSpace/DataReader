...
namespace CGEngine.Memory
{
    ...
    public sealed class RingMemoryStream : Stream
    {
        ...
        /// </summary>
        public RingMemoryStream () : this(INITCAPACITY)
        {
        }
        ...
        /// </summary>
        public RingMemoryStream (int capacity) : this(capacity, true)
        {
        }
        ...
        public RingMemoryStream (int capacity, bool expandable) : this(capacity, expandable, -1)
        {
        }
        ...
        public override int Read (byte[] buffer, int offset, int count)
        {
            ...
            lock (m_synObject)
            {
                ...
                ReadInternal(buffer, offset, readLen, true);
        ...
        /// <summary>
        /// 已重写。从当前流中读取字节块并将数据写入 buffer 中。。
        /// </summary>
        /// <param name="buffer">包含所读取到的字节。</param>
        /// <param name="offset">buffer 中的字节偏移量。</param>
        /// <param name="count">最多读取的字节数。</param>
        /// <param name="posmove">已读取的数据是否将被流抛弃</param>
        /// <returns>成功读取到的总字节数。如果没有读取到任何字节则为 0。</returns>             
        public int ReadData (byte[] buffer, int offset, int count, bool posmove)
        {
            if (buffer == null)
            {
                throw new ArgumentNullException("buffer");
            ...
            lock (m_synObject)
            {
                ...
                ReadInternal(buffer, offset, readLen, posmove);
        ...
        private void WriteInternal (byte[] buffer, int offset, int count)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Buffer.BlockCopy(buffer, offset + afterWritePosLen, m_buffer, 0, restLen);
                }
            }
            m_wPos += count;
            m_wPos %= m_capacity;
        ...
        private void ExpandStream (int minSize)
        {
            ...
            int strLen = m_length;
            ReadInternal(buffNew, 0, m_length, true);
            m_buffer = buffNew;
            m_rPos = 0;
            m_wPos = strLen;
